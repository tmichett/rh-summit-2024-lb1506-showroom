...
including PXE or HTTP Boot over the network, as an image via IMPI, or a phyiscal disk inserted into a drive. The kickstart 
...
booting from the custom iso. This process is not typical for virtual machines, but more common for bare metal hosts, especially in environments 
with limited network connectivity.  It shows how a container image can make it's way anywhere with a couple of simple commands and scripts. 
It's also important to note, everything needed is inside the custom iso created and at no point is network access required.
