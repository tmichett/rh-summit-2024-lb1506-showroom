...
How can we tell that we are on system and not running a container? First, `bootc` can tell you directly if it's being run 
...
We can see in the kernel command line some clear ties to `ostree`, and our PID1 details shows systemd running with init context from 
